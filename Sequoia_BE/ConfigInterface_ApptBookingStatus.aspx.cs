...
{
    public partial class ConfigInterface_ApptBookingStatus : System.Web.UI.Page
    {
        ...
        protected void Get_ApptBookingStatuses()
        {

            HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format(System.Configuration.ConfigurationManager.AppSettings["uri"] + "api/ApptBookingStatuses"));
            WebReq.Method = "GET";
            ...
            List<ApptBookingStatusUpdate> items = new List<ApptBookingStatusUpdate>();
            items = JsonConvert.DeserializeObject<List<ApptBookingStatusUpdate>>(jsonString);
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                Col_1.InnerHtml = "<a href='ConfigInterface_ApptBookingStatusMaster.aspx?PKey=" + items[i].bsCode + "'><font color='#6a6000'><b>" + items[i].bsCode.ToString() + "</b></font></a>";
                //Col_1.InnerText = items[i].bsCode;
                _Row.Controls.Add(Col_1);
                HtmlTableCell Col_2 = new HtmlTableCell();
                Col_2.Attributes.Add("style", "width: 35 %;");
                Col_2.InnerText = items[i].bsDesc;
                _Row.Controls.Add(Col_2);

                Col_2 = new HtmlTableCell();
                Col_2.Attributes.Add("style", "width: 25 %;");
                Col_2.InnerText = items[i].bsColorCode;
                _Row.Controls.Add(Col_2);
                ...
                if (items[i].active)
                {
                    Col_3.InnerText = "Yes"; // "<input type='checkbox' checked class='chk_ApptBookingStatus'>";
                }
                else
                {
                    Col_3.InnerText = "No";  // "<input type='checkbox' class='chk_ApptBookingStatus editor - active'>";
                }
                ...
                //Col_Action.InnerHtml = "<div class='tools'><i class='edit_ApptBookingStatus glyphicon glyphicon-pencil'></i></div>";
                ...
                //Col_4.Attributes.Add("style", "width: 0px; display:none");
                //_Row.Controls.Add(Col_4);
                tblApptBookingStatus.Rows.Add(_Row);
