...
{
    public partial class ItemUOMMaster : System.Web.UI.Page
    {
        ...
        protected void Get_ItemUOMs()
        {
            ...
            List<ItemUOMUpdate> items = new List<ItemUOMUpdate>();
            items = JsonConvert.DeserializeObject<List<ItemUOMUpdate>>(jsonString);
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                Col_1.InnerText = items[i].uomCode;
                _Row.Controls.Add(Col_1);
                HtmlTableCell Col_2 = new HtmlTableCell();
                Col_2.Attributes.Add("style", "width: 50 %;");
                Col_2.InnerText = items[i].uomDesc;
                _Row.Controls.Add(Col_2);
                HtmlTableCell Col_3 = new HtmlTableCell();
                Col_3.Attributes.Add("style", "width: 10 %; text-align:center");
                if (items[i].uomIsactive)
                {
                    Col_3.InnerHtml = "<input type='checkbox' checked class='chk_ItemUOM editor - active'>";
                }
                else
                {
                    Col_3.InnerHtml = "<input type='checkbox' class='chk_ItemUOM editor - active'>";
                }
                ...
                Col_Action.InnerHtml = "<div class='tools'><i class='edit_ItemUOM glyphicon glyphicon-pencil'></i></div>";
