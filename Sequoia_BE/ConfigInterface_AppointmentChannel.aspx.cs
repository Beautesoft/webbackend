    ...
    public partial class ConfigInterface_AppointmentChannel : System.Web.UI.Page
    {
        ...
        private void Get_ControlPrefixs()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                //GET Method  
                string api = "api/ControlNos?filter={\"where\":{\"controlDescription\":\"APPOINTMENT CHANNEL\"}}";
        ...
        protected void Get_AppointmentChannel()
        {
            ...
            HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format(System.Configuration.ConfigurationManager.AppSettings["uri"] + "api/ApptChannels"));
            WebReq.Method = "GET";
            ...
            List<ApptChannelUpdate> items = new List<ApptChannelUpdate>();
            items = JsonConvert.DeserializeObject<List<ApptChannelUpdate>>(jsonString);
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                Col_1.InnerText = items[i].channelCode;
                _Row.Controls.Add(Col_1);
                HtmlTableCell Col_2 = new HtmlTableCell();
                Col_2.Attributes.Add("style", "width: 50 %;");
                Col_2.InnerText = items[i].channelDesc;
                _Row.Controls.Add(Col_2);
                HtmlTableCell Col_3 = new HtmlTableCell();
                Col_3.Attributes.Add("style", "width: 10 %; text-align:center");
                if (items[i].active)
                {
                    Col_3.InnerHtml = "<input type='checkbox' checked class='chk_AppointmentChannel editor - active'>";
                }
                else
                {
                    Col_3.InnerHtml = "<input type='checkbox' class='chk_AppointmentChannel editor - active'>";
                }
                ...
                Col_Action.InnerHtml = "<div class='tools'><i class='edit_AppointmentChannel glyphicon glyphicon-pencil'></i></div>";
