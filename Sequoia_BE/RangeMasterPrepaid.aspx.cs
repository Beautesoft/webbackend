...
namespace Sequoia_BE
{
    public partial class RangeMasterPrepaid : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (Request.QueryString["PKey"] != null)
                {
                    using (var client1 = new HttpClient())
                    {
                        ...
                        //GET Method  
                        bool val = true;
                        string api = "api/itemBrands?filter={\"where\":{\"prepaidBrand\":\"" + val + "\",\"itmStatus\":\"True\"}}";
        ...
        [WebMethod]
        public static void AddRangeMasterPrepaidData(string itmCode, string itmDesc, bool itmStatus, string itmBrand, string siteCode)
        {
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    using (var client1 = new HttpClient())
                    {
                        ...
                        ControlNosUpdate c = new ControlNosUpdate { controldescription = "Range", sitecode= siteCode, controlnumber = Convert.ToString(NewcontrolNo) };
        ...
        public static void EditAddRangeMasterPrepaidData(string itmCode, string itmDesc, bool itmStatus, string itmBrand)
        {
            using (var client = new HttpClient())
            {
                RangeMasterProductInput p = new RangeMasterProductInput { itmCode = itmCode, itmDesc = itmDesc, itmStatus = itmStatus, itmBrand = itmBrand, isproduct = true, prepaidForProduct = false, prepaidForService = false, prepaidForAll = false, isservice = false, isvoucher = false, isprepaid = true, iscompound = false };
                client.BaseAddress = new Uri(System.Configuration.ConfigurationManager.AppSettings["uri"]);
                client.DefaultRequestHeaders.Accept.Add(new System.Net.Http.Headers.MediaTypeWithQualityHeaderValue("application/json"));
